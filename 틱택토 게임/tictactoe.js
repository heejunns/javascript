...
const clcikBlock = (event) => {
  ...
  if ($resultChange.textContent == '') {
    ...
    alert('게임을 다식 시작하려면 게임 다시 시작하기 버튼을 클릭하세요.');
  }
};
for (let i = 0; i < 3; ++i) {
...
$table.addEventListener('click', clcikBlock); // 이벤트 버블링 현상으로 자식 태그를 클릭해도 부모 태그로 올라가면 이벤트 발생
...
$reset.addEventListener('click', clcikReset);
