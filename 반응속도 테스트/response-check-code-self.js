...
const clickScreen = () => {
  ...
  if ($screen.className === 'wait') {
    ...
    records.sort((a, b) => a - b); // 오름차순으로 정렬하기
    const average =
    ...
    for (let i = 0; i < 5; ++i) {
      if (records[i] === undefined) {
        // undefined 라면 for 문 종료
        break;
      }
      if (i == 0) {
        // 인덱스 0
        $rank1.textContent = `${records[i]} ms`;
      } else if (i == 1) {
        // 인덱스 1
        $rank2.textContent = `${records[i]} ms`;
      } else if (i == 2) {
        // 인덱스 2
        $rank3.textContent = `${records[i]} ms`;
      } else if (i == 3) {
        // 인덱스 3
        $rank4.textContent = `${records[i]} ms`;
      } else if (i == 4) {
        // 인덱스 4
        $rank5.textContent = `${records[i]} ms`;
      }
    }
    startTime = null;
    endTime = null;
