...
let cell; // 칸
let mine; // 지뢰 개수
let openCount; // 오픈한 칸의 개수를 기록할 변수
let startTime; // 시작 시간을 기록할 변수
let interval; // 화면에 1초 간격으로 시간이 가는것을 보여주기 위해 setTimeIneterval 함수를 사용 하였는데 게임이 종료되면 시간을 멈춰야 하니까 이 함수를 종료하기 위한 반환값을 저장할 변수
...
function showMine() {
  showMineList = [CODE.MINE, CODE.FLAG_MINE, CODE.QUESTION_MINE];
  data.flat().forEach((element, index) => {
    if (showMineList.includes(element)) {
      $tbody.children[Math.floor(index / cell)].children[
        index % cell
      ].textContent = '*';
    }
  });
...
let findBlock = false;
function mineMove(rowIndex, colIndex) {
  if (findBlock) return;
  if (data[rowIndex]?.[colIndex] == undefined) return;
  if ($tbody.children[rowIndex].children[colIndex].classList.contains('check'))
    return;
  if (data[rowIndex][colIndex] == CODE.NORMAL) {
    findBlock = true;
    data[rowIndex][colIndex] = CODE.MINE;
    console.log(rowIndex, colIndex);
    $tbody.children[rowIndex].children[colIndex].textContent = '*';
  } else {
    $tbody.children[rowIndex].children[colIndex].classList.add('check');
    mineMove(rowIndex, colIndex - 1);
    mineMove(rowIndex - 1, colIndex - 1);
    mineMove(rowIndex - 1, colIndex);
    mineMove(rowIndex - 1, colIndex + 1);
    mineMove(rowIndex, colIndex + 1);
    mineMove(rowIndex + 1, colIndex + 1);
    mineMove(rowIndex + 1, colIndex);
...
const clickLeftBlock = function (event) {
  ...
  if (firstClick) {
    firstClick = false;
    if (data[rowIndex][colIndex] === CODE.MINE) {
      mineMove(rowIndex, colIndex);
...
const onSubmit = function (event) {
  ...
  drawTable(); // drawTable 함수 호출 칸 화면에 그리기
